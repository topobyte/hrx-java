...
public class TestEmptyFile
{
	...
	{
		expected.add(HrxFiles.comment(TestUtil.lines("This file is empty.")));
		expected.add(HrxFiles.file("file1", TestUtil.lines()));
		expected.add(HrxFiles.comment(TestUtil.lines("So is this one.")));
	...
	{
		try (Reader reader = Resources.asReader("examples/empty-file.hrx")) {
